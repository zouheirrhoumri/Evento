...
<body>
    ...
    <div class="min-h-screen bg-gray-100 p-0 sm:p-12">
        <div class="mx-auto max-w-md px-6 py-12 bg-white border-0 shadow-lg sm:rounded-3xl">
            <h1 class="text-2xl font-bold mb-8">Edit your Event</h1>
            <form method="POST" action="{{ route('events.update', ['eventId' => $event->id]) }}">
                @csrf
                @method('PUT')
                <div class="relative z-0 w-full mb-5">
                    <input type="text" name="title" value="{{ $event->title }}" placeholder="" required
                    ...
                    <textarea type="text" name="description" placeholder="Description" required
                        class="pt-3 pb-2 block w-full px-0 mt-0 bg-transparent border-0 border-b-2
                         appearance-none focus:outline-none focus:ring-0
                          focus:border-black border-gray-200"> {{ $event->description }} </textarea>
                    <label for="name" class="absolute duration-300 top-3 -z-1 origin-0 text-gray-500">Event's
                        Description</label>
                </div>

                <div class="relative z-0 w-full mb-5">
                    <input type="date" value="{{ $event->date }}" name="date" placeholder=" "
                        class="pt-3 pb-2 block w-full px-0 mt-0 bg-transparent border-0 border-b-2 appearance-none focus:outline-none focus:ring-0 focus:border-black border-gray-200" />
                    <label for="date" class="absolute duration-300 top-3 -z-1 origin-0 text-gray-500">Date</label>
                </div>

                <div class="relative z-0 w-full mb-5">
                    <input type="text" value="{{ $event->location }}" name="location" placeholder=" "
                        class="pt-3 pb-2 block w-full px-0 mt-0 bg-transparent border-0 border-b-2 appearance-none focus:outline-none focus:ring-0 focus:border-black border-gray-200" />
                    <label for="location"
                ...
                <div class="relative z-0 w-full mb-5">
                    <input type="number" value="{{ $event->available_places }}" name="available_places" placeholder=" "
